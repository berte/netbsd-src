...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: if_bwfm_sdio.c,v 1.18 2017/10/08 10:11:32  Exp $");
...
void		bwfm_sdio_attach(device_t, device_t, void *);
...
#else
CFATTACH_DECL_NEW(bwfm_sdio, sizeof(struct bwfm_sdio_softc), bwfm_sdio_match, bwfm_sdio_attach, bwfm_sdio_detach, NULL);
#endif
...
bwfm_sdio_attach(device_t parent, device_t self, void *aux)
{
	...
	sc->sc_sf = malloc((sf->sc->sc_function_count + 1) * sizeof(struct sdmmc_function *), 
			M_DEVBUF, M_WAITOK);
