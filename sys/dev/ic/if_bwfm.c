...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: if_bwfm.c ,v 1.16 2017/10/03 20:10:00  Exp $");
...
int	 bwfm_fwvar_cmd_get_data(struct bwfm_softc *, int, char *, size_t);
int	 bwfm_fwvar_cmd_set_data(struct bwfm_softc *, int, char *, size_t);
int	 bwfm_fwvar_cmd_get_int(struct bwfm_softc *, int, uint32_t *);
int	 bwfm_fwvar_cmd_set_int(struct bwfm_softc *, int, uint32_t);
int	 bwfm_fwvar_var_get_data(struct bwfm_softc *, const char *, char *, size_t);
int	 bwfm_fwvar_var_set_data(struct bwfm_softc *, const char *, char *, size_t);
...
bwfm_attach(struct bwfm_softc *sc)
{
	...
	/* Init some net80211 stuff we take over. */
//	RBT_INIT(ieee80211_tree, &ic->ic_tree);
	...
	memcpy(enaddr, ic->ic_myaddr, ETHER_ADDR_LEN);
...
bwfm_ioctl(struct ifnet *ifp, u_long cmd, void* data)
{
	...
	case SIOCG80211ALLNODES:
		...
		/*ni = ieee80211_find_node(&ic->ic_scan, nr->nr_macaddr);
		while (ni && na->na_size >=
	...
	case SIOCS80211NODE:
		...
		//ni = ieee80211_find_node(ic, nr->nr_macaddr);
		ni = ieee80211_find_node(&ic->ic_sta, ic->ic_myaddr);
		if (ni == NULL)
    ...
    int cmd, char *buf, size_t *len)
{
	...
	do {
		...
		dcmd->hdr.cmd = htole32(dcmd->hdr.cmd);
		dcmd->hdr.len = htole32(dcmd->hdr.len);
		dcmd->hdr.flags = htole32(dcmd->hdr.flags);
		dcmd->hdr.status = htole32(dcmd->hdr.status);
...
/* FW Variable code */
int
bwfm_fwvar_cmd_get_data(struct bwfm_softc *sc, int cmd, char *data, size_t len)
{
	return sc->sc_proto_ops->proto_query_dcmd(sc, 0, cmd, data, &len);
}

int
bwfm_fwvar_cmd_set_data(struct bwfm_softc *sc, int cmd, char *data, size_t len)
{
...
{
	int ret;
	ret = bwfm_fwvar_cmd_get_data(sc, cmd, (void *)data, sizeof(*data));
	*data = htole32(*data);
	return ret;
}
...
{
	data = htole32(data);
	return bwfm_fwvar_cmd_set_data(sc, cmd, (void *)&data, sizeof(data));
}

int
bwfm_fwvar_var_get_data(struct bwfm_softc *sc, const char *name, char *data, size_t len)
{
	char *buf;
	int ret;
...
int
bwfm_fwvar_var_set_data(struct bwfm_softc *sc, const char *name, char *data, size_t len)
{
	char *buf;
	int ret;
...
{
	int ret;
	ret = bwfm_fwvar_var_get_data(sc, name, (void *)data, sizeof(*data));
	*data = htole32(*data);
	return ret;
}
...
{
	data = htole32(data);
	return bwfm_fwvar_var_set_data(sc, name, (void *)&data, sizeof(data));
}
...
bwfm_scan(struct bwfm_softc *sc)
{
	...
	bwfm_fwvar_var_set_data(sc, "escan", (void *)params, params_size);
...
bwfm_rx_event(struct bwfm_softc *sc, char *buf, size_t len)
{
	...
	case BWFM_E_ESCAN_RESULT: {
		...
		len -= sizeof(*e);
		if (len < sizeof(*res) || len < htole32(res->buflen)) {
			printf("%s: results too small\n", DEVNAME(sc));
			return;
		}
		len -= sizeof(*res);
		//if (len < letoh16(res->bss_count) * sizeof(struct bwfm_bss_info)) {
		if (len < htole16(res->bss_count) * sizeof(struct bwfm_bss_info)) {
			printf("%s: results too small\n", DEVNAME(sc));
			return;
		}
		bss = &res->bss_info[0];
		//for (i = 0; i < letoh16(res->bss_count); i++) {
		for (i = 0; i < htole16(res->bss_count); i++) {
			bwfm_scan_node(sc, &res->bss_info[i], len);
			len -= sizeof(*bss) + htole32(bss->length);
			bss = (void *)((char *)bss + htole32(bss->length));
			if (len <= 0)
				break;
		}
		break;
	...
	default:
		printf("%s: buf %p len %lu datalen %u code %u\n",
...
bwfm_scan_node(struct bwfm_softc *sc, struct bwfm_bss_info *bss, size_t len)
{
	...
	ni = ieee80211_alloc_node(&ic->ic_sta, ic->ic_myaddr);//bss->bssid);
