...
int		 bwfm_usb_load_microcode(struct bwfm_usb_softc *, u_char *,
		     size_t);
...
//static int	 usbd_is_dying(struct usb_softc *sc);
...
#else
CFATTACH_DECL_NEW(bwfm_usb, sizeof(struct bwfm_usb_softc), bwfm_usb_match, bwfm_usb_attach, bwfm_usb_detach, NULL);

/*struct cfattach  bwfm_usb_ca = {
		.ca_name		= "bwfm_usb",
		.ca_devsize		= sizeof(struct bwfm_usb_softc),
		.ca_flags		= 0,
		.ca_match 		= bwfm_usb_match,
		.ca_attach		= bwfm_usb_attach,
		.ca_detach		= bwfm_usb_detach,
		.ca_activate		= NULL,
		.ca_rescan		= NULL,
...
/*int usbd_is_dying(struct usb_softc *sc)
{
	return (sc->dying || sc->bus->dying);
}*/
...
bwfm_usb_attachhook(device_t self)
{
	...
	sc->sc_chip = htole32(brom.chip);
	sc->sc_chiprev = htole32(brom.chiprev);
	...
	if (sc->sc_chip != BRCMF_POSTBOOT_ID) {
		...
		error = firmware_load("bwfm", name, &ucode, &size);
		if (error != 0) {
			printf("%s: error %d, could not read firmware file %s\n", 
					DEVNAME(sc), error, name);
			return;
		}
		...
		for (i = 0; i < 10; i++) {
			...
			memset(&brom, 0, sizeof(brom));
			bwfm_usb_dl_cmd(sc, DL_GETVER, &brom, sizeof(brom));
			if (htole32(brom.chip) == BRCMF_POSTBOOT_ID)
				break;
		}
...
int
bwfm_usb_load_microcode(struct bwfm_usb_softc *sc, u_char *ucode, size_t size)
{
	...
	if (htole32(trx->magic) != TRX_MAGIC ||
	    (htole32(trx->flag_version) & TRX_UNCOMP_IMAGE) == 0) {
		printf("%s: invalid firmware\n", DEVNAME(sc));
		return 1;
	}

	bwfm_usb_dl_cmd(sc, DL_START, &state, sizeof(state));
	rdlstate = htole32(state.state);
	rdlbytes = htole32(state.bytes);
